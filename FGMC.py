...
now = (now[:10]+"_"+now[11:16])
logging.basicConfig(filename=r"log path", level=logging.INFO)
...
def disc_process(dataFilePath):
    ...
    fgmcFolder = dataFilePath
    print(fgmcFolder)

    FGMC1 = find('*FGMC*', fgmcFolder)
    FGMC2 = find('*FGMC_2*', fgmcFolder)
    FGMC1 = FGMC1.replace('/', '\\')
    FGMC2 = FGMC2.replace('/', '\\')

    
    try:
        df1 = pd.read_csv(FGMC1, sep='|', dtype={'Loan Number': str})
        df2 = pd.read_csv(FGMC2, sep='|')
    except:
        tkinter.messagebox.showerror('File Reading Error','Please make sure the FGMC data file are closed and not in use.')
